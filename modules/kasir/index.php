...
<div x-data="posApp()" class="h-full flex flex-col md:flex-row gap-6 relative">
    ...
    <!-- ==================== MODAL: TAMBAH/UBAH MENU (kasir) ==================== -->
    <div x-show="showMenuForm" @keydown.escape.window="showMenuForm=false" class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-vibe-on-surface/40 backdrop-blur-sm" x-transition.opacity.duration.200ms style="display:none">
        <div @click.stop class="bg-white rounded-2xl w-full max-w-xl border border-vibe-outline-variant shadow-2xl max-h-[90vh] flex flex-col"
             x-show="showMenuForm"
            ...
            <div class="flex items-center justify-between px-6 py-5 border-b border-vibe-outline-variant shrink-0">
                <div>
                    <h3 class="text-xl font-display font-bold text-vibe-on-surface" x-text="menuForm.id ? 'Ubah Menu' : 'Tambah Menu'"></h3>
                    <p class="text-sm text-vibe-on-surface-variant mt-1">Lengkapi detail menu. Harga jual bebas, tercatat di audit.</p>
                </div>
                <button @click="showMenuForm=false" class="p-2 rounded-lg text-vibe-on-surface-variant hover:text-vibe-on-surface hover:bg-vibe-surface-dim transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
            </div>
            ...
            <div class="flex-1 overflow-y-auto min-h-0 px-6 py-5 space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="md:col-span-2">
                        <label class="block text-[11px] font-bold text-vibe-on-surface-variant uppercase tracking-wider mb-1.5">Nama Menu</label>
                        <input type="text" x-model="menuForm.nama_menu" placeholder="Contoh: Es Kopi Susu" class="w-full px-4 py-3 bg-white border border-vibe-outline-variant rounded-xl focus:outline-none focus:border-vibe-primary text-sm transition-colors">
                    </div>
                    <div>
                        <label class="block text-[11px] font-bold text-vibe-on-surface-variant uppercase tracking-wider mb-1.5">Kategori</label>
                        <select x-model="menuForm.kategori_id" class="w-full px-4 py-3 bg-white border border-vibe-outline-variant rounded-xl focus:outline-none focus:border-vibe-primary text-sm transition-colors">
                        ...
                        <div class="flex gap-2 mt-2">
                            ...
                            <button type="button" @click="addKasirKategori()" :disabled="savingKategori || !newKategoriName.trim()"
                                    class="px-3 py-2 rounded-lg bg-vibe-on-surface text-white text-xs font-bold disabled:opacity-50 transition-colors">
                                Tambah
                            </button>
                        </div>
                    </div>
                    <div>
                        <label class="block text-[11px] font-bold text-vibe-on-surface-variant uppercase tracking-wider mb-1.5">Harga Jual</label>
                        <div class="relative">
                            <span class="absolute left-4 top-1/2 -translate-y-1/2 text-vibe-on-surface-variant text-sm font-black">Rp</span>
                            <input type="number" step="1" min="0" x-model="menuForm.harga" placeholder="0"
                ...
                <div class="rounded-2xl border border-vibe-outline-variant/50 bg-vibe-surface-dim/40 p-4">
                    ...
                    <div class="space-y-2">
                        <template x-for="(item, index) in menuForm.items" :key="index">
                            <div class="grid grid-cols-1 md:grid-cols-[1fr_120px_60px_36px] gap-2 items-center bg-white border border-vibe-outline-variant/70 rounded-xl p-3">
                                <div>
                                ...
                                <div class="px-2 py-2.5 bg-vibe-surface-dim rounded-lg text-sm font-bold text-vibe-on-surface-variant text-center" x-text="item.satuan || '—'"></div>
                                <button type="button" @click="removeMenuIngredient(index)" class="h-10 w-9 rounded-lg text-vibe-on-surface-variant hover:text-vibe-error hover:bg-vibe-error-container transition-colors">
            ...
            <div class="px-6 py-5 border-t border-vibe-outline-variant bg-white flex gap-3 shrink-0">
                <button type="button" @click="showMenuForm=false" class="btn-press flex-1 py-3 rounded-xl border border-vibe-outline-variant/60 text-vibe-on-surface-variant font-bold text-sm hover:text-vibe-on-surface hover:bg-vibe-surface-dim transition-colors">Batal</button>
                <button type="button" @click="saveKasirMenu()" :disabled="savingMenu" class="btn-press flex-1 py-3 rounded-xl bg-vibe-primary text-white font-black text-sm hover:bg-vibe-primary-container transition-colors disabled:opacity-60 shadow-sm shadow-vibe-primary/20">
                    <span x-show="!savingMenu" x-text="menuForm.id ? 'Simpan Perubahan' : 'Simpan Menu'"></span>
                    <span x-show="savingMenu" class="inline-flex items-center justify-center gap-2">
                        <svg class="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                        Menyimpan...
                    </span>
                </button>
