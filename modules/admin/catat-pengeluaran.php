...
<div x-data="catatPengeluaran()" class="max-w-3xl mx-auto space-y-0">
    ...
    <!-- Back Navigation -->
    <a href="<?= BASE_URL ?>/modules/admin/keuangan.php" class="btn-press inline-flex items-center gap-2 text-sm font-semibold text-vibe-on-surface-variant hover:text-vibe-on-surface transition-colors mb-6 group">
        ...
        Kembali ke Administrasi Keuangan
    </a>

    <!-- Page Title -->
    <div class="mb-8">
        <h1 class="text-2xl font-display font-bold text-vibe-on-surface tracking-tight" x-text="editId ? 'Ubah Pengeluaran' : 'Catat Pengeluaran Baru'"></h1>
        <p class="text-sm text-vibe-on-surface-variant mt-1">Isi detail pengeluaran untuk pembukaan, operasional, atau lainnya.</p>
    </div>
    ...
    Alpine.data('catatPengeluaran', () => ({
        ...
        async submitForm() {
            ...
            try {
                ...
                window.location.href = '<?= BASE_URL ?>/modules/admin/keuangan.php';
            } catch (err) {
