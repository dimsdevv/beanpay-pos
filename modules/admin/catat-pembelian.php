...
<div x-data="catatPembelian()" class="max-w-3xl mx-auto space-y-0">

    <!-- Back Navigation -->
    <a href="<?= BASE_URL ?>/modules/admin/pembelian-bahan.php" class="btn-press inline-flex items-center gap-2 text-sm font-semibold text-vibe-on-surface-variant hover:text-vibe-on-surface transition-colors mb-6 group">
    ...
    <!-- Page Title -->
    <div class="mb-8">
        <h1 class="text-2xl font-display font-bold text-vibe-on-surface tracking-tight" x-text="editId ? 'Ubah Pembelian' : 'Catat Pembelian Baru'"></h1>
        <p class="text-sm text-vibe-on-surface-variant mt-1">Isi detail belanja bahan baku. Harga modal akan dihitung otomatis.</p>
    </div>

    <!-- ═══════════ SECTION 1: Info Pembelian ═══════════ -->
    <div class="bg-white border border-vibe-outline-variant/50 rounded-2xl p-6 mb-4">
        <h2 class="text-xs font-bold text-vibe-on-surface-variant uppercase tracking-widest mb-5">Informasi Pembelian</h2>
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
                <label class="block text-sm font-semibold text-vibe-on-surface mb-1.5">Supplier / Toko</label>
                <input type="text" x-model="form.supplier" placeholder="Mis. Toko Makmur" class="w-full px-4 py-3 bg-white border border-vibe-outline-variant rounded-xl focus:outline-none focus:border-vibe-on-surface text-sm transition-colors placeholder-vibe-outline-variant">
            </div>
            <div>
    ...
    <div class="bg-white border border-vibe-outline-variant/50 rounded-2xl p-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div class="flex gap-3 w-full sm:w-auto">
                <a href="<?= BASE_URL ?>/modules/admin/pembelian-bahan.php" class="btn-press flex-1 sm:flex-initial flex items-center justify-center py-3.5 px-6 rounded-xl border border-vibe-outline-variant/60 text-vibe-on-surface-variant font-bold text-sm hover:text-vibe-on-surface hover:bg-vibe-surface-dim transition-colors">
                    Batal
                </a>
                <button type="button" @click="submitForm()" :disabled="saving" class="btn-press flex-1 sm:flex-initial flex items-center justify-center gap-2 py-3.5 px-8 rounded-xl bg-vibe-primary text-white font-bold text-sm hover:bg-vibe-primary-container transition-colors disabled:opacity-60" x-text="saving ? 'Menyimpan…' : (editId ? 'Simpan Perubahan' : 'Simpan Pembelian')">
                </button>
            </div>
        </div>
    </div>
    ...
    Alpine.data('catatPembelian', () => ({
        ...
        async submitForm() {
            ...
            try {
                ...
                window.location.href = '<?= BASE_URL ?>/modules/admin/pembelian-bahan.php';
            } catch (err) {
