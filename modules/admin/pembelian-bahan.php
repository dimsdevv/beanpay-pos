...
.hide-scrollbar {
    ...
    <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
        <div>
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-2xl font-display font-bold text-vibe-on-surface tracking-tight">Pembelian Bahan Baku</h1>
                    <p class="text-sm text-vibe-on-surface-variant mt-1">Catat belanja bahan baku untuk menu cafe.</p>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white border border-vibe-outline-variant/60 rounded-2xl overflow-hidden animate-fade-up mt-8 shadow-sm">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 border-b border-vibe-outline-variant/50">
            <div>
                <h3 class="font-display text-lg font-bold text-vibe-on-surface">Riwayat Pembelian Bahan</h3>
                <p class="text-xs text-vibe-on-surface-variant mt-1">Daftar semua pembelian bahan baku pada <?= $periodeLabel ?></p>
            </div>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('keuanganApp', () => ({
        ...
        // ---- Open ----
        openAdd() {
            this.form = { id: null, tanggal: '<?= date('Y-m-d') ?>', supplier: '', kategori: 'pembukaan', metode_bayar: 'cash', keterangan: '', stok_updated: true, buktiName: '', items: [] };
            this.buktiFile = null;
            this.addItem();
            this.showForm = true;
        },
