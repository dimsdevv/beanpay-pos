...
.hide-scrollbar {
    ...
    <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
        <div>
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-2xl font-display font-bold text-vibe-on-surface tracking-tight">Barang Operasional</h1>
                    <p class="text-sm text-vibe-on-surface-variant mt-1">Catat barang-barang non-menu seperti peralatan, kebersihan, dan lain-lain.</p>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white border border-vibe-outline-variant/60 rounded-2xl overflow-hidden animate-fade-up mt-8 shadow-sm">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 border-b border-vibe-outline-variant/50">
            <div>
                <h3 class="font-display text-lg font-bold text-vibe-on-surface">Riwayat Barang Operasional</h3>
                <p class="text-xs text-vibe-on-surface-variant mt-1">Daftar semua pembelian barang operasional pada <?= $periodeLabel ?></p>
            </div>
...
<div x-show="showForm" @keydown.escape.window="showForm=false" class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-vibe-surface-dim/80 backdrop-blur-sm" x-transition.opacity.duration.200ms style="display:none">
    <div @click.stop class="bg-white rounded-2xl w-full max-w-2xl border border-vibe-outline-variant/50 max-h-[90vh] flex flex-col shadow-2xl"
        ...
        <div class="flex items-center justify-between px-6 py-5 border-b border-vibe-outline-variant/50 shrink-0">
            <div>
                <h3 class="text-lg font-display font-bold text-vibe-on-surface" x-text="form.id ? 'Ubah Barang' : 'Catat Barang Operasional'"></h3>
                <p class="text-xs text-vibe-on-surface-variant">Masukkan detail barang operasional (peralatan, kebersihan, dll).</p>
            </div>
        ...
        <div class="flex-1 overflow-y-auto min-h-0 px-6 py-5 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <div>
                    ...
                    <div class="px-3 py-2.5 bg-vibe-surface-dim border border-vibe-outline-variant rounded-lg text-sm font-bold text-vibe-on-surface">Operasional</div>
                    <input type="hidden" x-model="form.kategori" value="operasional">
                </div>
            </div>
            ...
            <div>
                <div class="flex items-center justify-between mb-2">
                    <label class="text-[11px] font-bold text-vibe-on-surface-variant uppercase tracking-wider">Item Barang</label>
                ...
                <div class="space-y-2">
                    <template x-for="(it, idx) in form.items" :key="idx">
                        <div class="flex flex-col sm:flex-row sm:items-end gap-2 bg-white border border-vibe-outline-variant/50 rounded-lg p-2.5 transition-all">
                            ...
                            <div class="w-full sm:flex-1">
                                <label class="block sm:hidden text-[10px] font-bold text-vibe-on-surface-variant uppercase mb-1">Nama Barang</label>
                                <input type="text" x-model="it.nama_bahan" placeholder="Contoh: Dispenser, Tong Sampah, dll." class="w-full px-3 py-2 bg-white border border-vibe-outline-variant rounded-md focus:outline-none focus:border-vibe-primary text-sm transition-colors">
                            </div>
                    ...
                    <div x-show="form.items.length === 0" class="text-center py-4 text-xs text-vibe-on-surface-variant bg-vibe-surface-dim rounded-lg">
                        Belum ada item barang. Klik "Tambah item" untuk mulai mencatat.
                    </div>
                </div>
